---
layout: default
title: Teaching
permalink: /teaching/
---

# Teaching

## Lý thuyết Mạch điện 1

(In Vietnamese) Circuit Theory 1: Steady-State Analysis of Linear Circuit

#### Thông tin môn học

Mã học phần: EE2021

Ngành EE1 (Điện) và EE2 (Điều khiển và Tự động hoá) Trường Điện - Điện tử, ĐH Bách Khoa HN

Nội dung: Mạch điện tuyến tính ở chế độ xác lập

#### Bài giảng

<ul>
  <li>
    <a href="{{ '/assets/files/teaching/Baigiang_LTM1_BHN.pdf' | relative_url }}" target="_blank" rel="noopener">
      [Download] Slides bài giảng LTM1
    </a>
  </li>
</ul>

<iframe
  src="{{ '/assets/files/teaching/Baigiang_LTM1_BHN.pdf' | relative_url }}"
  width="100%"
  height="700px"
  style="border: 1px solid #d9ded7; border-radius: 6px;">
</iframe>

## Lý thuyết Mạch điện 2

(In Vietnamese) Circuit Theory 2: Steady-State and Transient Analysis of Nonlinear Circuit

#### Thông tin môn học

Mã học phần: EE2022 

Ngành EE1 (Điện) và EE2 (Điều khiển và Tự động hoá) Trường Điện - Điện tử, ĐH Bách Khoa HN

Nội dung: Mạch điện tuyến tính ở chế độ quá độ, mạch điện phi tuyến, và đường dây dài

#### Bài giảng

<ul>
  <li>
    <a href="{{ '/assets/files/teaching/Baigiang_LTM2_BHN.pdf' | relative_url }}" target="_blank" rel="noopener">
      [Download] Slides bài giảng LTM2
    </a>
  </li>
</ul>

<iframe
  src="{{ '/assets/files/teaching/Baigiang_LTM2_BHN.pdf' | relative_url }}"
  width="100%"
  height="700px"
  style="border: 1px solid #d9ded7; border-radius: 6px;">
</iframe>

## Lý thuyết Trường điện từ

(In Vietnamese) Electromagnetics

#### Thông tin môn học

Mã học phần: EE2031 

Ngành EE2 (Điều khiển và Tự động hoá) Trường Điện - Điện tử, ĐH Bách Khoa HN

Nội dung: Giải tích vector, điện trường tĩnh, điện trường dừng, từ trường tĩnh, trường điện từ biến thiên

#### Bài giảng

<ul>
  <li>
    <a href="{{ '/assets/files/teaching/Baigiang_LTTDT_BHN.pdf' | relative_url }}" target="_blank" rel="noopener">
      [Download] Slides bài giảng LTT
    </a>
  </li>
</ul>

<iframe
  src="{{ '/assets/files/teaching/Baigiang_LTTDT_BHN.pdf' | relative_url }}"
  width="100%"
  height="700px"
  style="border: 1px solid #d9ded7; border-radius: 6px;">
</iframe>
